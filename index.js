...
async function authenticate() {
  ...
  const data = await httpJson("POST", url, { body: { username: USER, password: PASS } });
  ...
  cachedToken = token;
  tokenExpiresAt = Date.now() + Math.max(60, expiresInSec - 30) * 1000;

  console.log("[AUTH] token cacheado ~", expiresInSec, "s");
  return token;
}
